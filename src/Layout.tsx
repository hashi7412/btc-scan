...
const Layout = (props: any) => {
	...
	const { loading, T, theme, update, account, logout, showLoading, walletInfo } = useStore()
	...
	const onAddNetwork = async () => {
		showLoading(true)
		await addNetwork(config.chain, config.symbol, config.chainId, config.rpc, "https://hirokiscan.io")
		showLoading(false)
	}
								...
								<ul className="menu">
									...
									<li className='sub-right'>
										...
										<div className="sub-menu">
											<div className='common'>
												<div className='flex column'>
													{config.mainscan!=='' && <a href={config.mainscan}>{config.symbol} {T('menu.network.main')}</a>}
													{config.testscan!=='' && <a href={config.testscan}>{config.symbol} {T('menu.network.test')}</a>}
												</div>
											</div>
										</div>
									</li>
				...
				<footer>
					<section className='container'>
						<div className="footer-content">
							...
							<div>
								<div>
									<button className='btn btn-info d-middle g1' onClick={()=>onAddNetwork()}><img src="/logo.svg" alt="metamask" width={18} height={18} />Add {config.chain} {config.testnet ? 'Testnet' : 'Mainnet'}</button>
